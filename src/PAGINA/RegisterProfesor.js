...
// Usamos variable de entorno con fallback a localhost
...
export default function RegisterProfesor() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!token && !firstAdmin) return setMsg("No hay token de admin disponible");

    const { nombre, edad, sexo, email, password } = form;
    if (!nombre || !edad || !sexo || !email || !password) return setMsg("Todos los campos son obligatorios");

    if (!email.toLowerCase().endsWith("@gmail.com") && !email.toLowerCase().endsWith("@iea.edu.mx"))
    ...
    try {
      ...
      Object.keys(finalForm).forEach((key) => formData.append(key, finalForm[key]));
      ...
      setMsg(err.response?.data?.msg || "Error ya existe la cuenta: " + err.message);
    } finally {
